...
export default function RootLayout({
  ...
  return (
    <html lang="ko">
      <body>
        <div className="relative w-full max-w-[500px] h-screen mx-auto bg-gradient-to-b from-[#F3F3F3] to-[#F3F3F3]">
          {/* Main Content */}
          <div className="relative h-[calc(100%-60px)] sm:h-[calc(100%-72px)]">
            {children}
          </div>

          {/* Bottom Navigation Bar */}
          <nav className="absolute bottom-0 left-0 right-0 w-full h-[60px] sm:h-[72px] bg-white border-t border-[#F3EFEC] flex justify-around items-center px-4 sm:px-10">
            {/* 전화 걸기 */}
            <button
              onClick={() => router.push("/")}
              className="flex flex-col justify-center items-center gap-0.5 sm:gap-[2px] flex-1 h-full"
            >
              <div className="w-6 h-6 sm:w-7 sm:h-7 relative">
                <svg
                  ...
                  viewBox="0 0 24 24"
                  fill="none"
                  className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 sm:w-6 sm:h-6"
                >
                  <path
                    ...
                    fill={pathname === "/" ? "#3A3935" : "#A7A39F"}
                  />
                </svg>
              </div>
              <span
                className={`text-xs sm:text-sm font-semibold text-center tracking-[-0.03em] ${
                  pathname === "/" ? "text-[#3A3935]" : "text-[#A7A39F]"
                }`}
              >
                전화 걸기
              </span>
            ...
            >
              <div className="w-6 h-6 sm:w-7 sm:h-7 relative">
                ...
                >
                  <circle
                    ...
                    cy="8.5"
                    r="5.38"
                    fill={pathname === "/profile" ? "#3A3935" : "#A7A39F"}
                  />
                  <path
              ...
              <span
                className={`text-xs sm:text-sm font-semibold text-center tracking-[-0.03em] ${
                  pathname === "/profile" ? "text-[#3A3935]" : "text-[#A7A39F]"
                }`}
              >
                나
              </span>
